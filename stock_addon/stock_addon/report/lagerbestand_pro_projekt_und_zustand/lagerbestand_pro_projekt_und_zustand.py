...
def execute(filters=None):
	...
	for project in project_list:
		...
		for item in item_list:
			...
			print("===",f"""select qty_after_transaction as qty, posting_date from `tabStock Ledger Entry` where item_code='{item}'{project_conditions} {where} order by posting_date desc LIMIT 1""")
			new_serial = frappe.db.sql(f"""select count(zustand='Neu') as qty from `tabSerial No` where item_code='{item}'{project_conditions} and zustand='Neu' {zustand_where}""",as_dict=1)
			used_serial = frappe.db.sql(f"""select count(zustand='Gebrauch') as qty from `tabSerial No` where item_code='{item}'{project_conditions} and zustand='Gebrauch' {zustand_where}""",as_dict=1)
			defected_serial = frappe.db.sql(f"""select count(zustand='Defekt') as qty from `tabSerial No` where item_code='{item}'{project_conditions} and zustand='Defekt' {zustand_where}""",as_dict=1)
			if qty_date:
				data.append({
					"item_code": item,
					"project": project,
